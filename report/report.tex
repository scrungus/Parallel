...
Running on 44 threads with an matrix of dimension $n=1000$, the program completes in ~63 seconds. The same size matrix completes sequentially in 12 minutes 35.5 seconds\footnote{on average, over 10 runs.}. This gives us an efficiency of:
\begin{figure}[H]
    \centering
...
- move cells to global context
- create pointer in control thread to each cell
- populate cells 
- give each thread a start id and end id
- offload work discovery to threads
